...
DEFAULT_CONFIG = {
    ...
    # County filtering — alerts that don't mention ANY of these counties are skipped.
    # Case-insensitive substring match against the alert's areaDesc.
    # Set to [] to disable county filtering and receive all zone/state alerts.
    "counties": ["Douglas", "Sarpy"],

    # Discord targeting
    "alert_channel_id":     0,
    "product_channel_id":   0,
    "ping_role_id":         0,          # role to ping for non-everyone alerts
    "everyone_events":      ["Tornado Emergency", "Tornado Warning"],

    # Alert filters — which NWS event types to post to Discord AND trigger notifications.
    # Toggling an event off here disables it EVERYWHERE: embeds, SMS, ntfy, pings.
    "enabled_alerts": {
    ...
    # Which enabled alerts also trigger SMS  (must also be True in enabled_alerts)
    "sms_events": [
        "Tornado Emergency",
        "Tornado Warning",
        "Tornado Watch",
    ...
    # Which enabled alerts also trigger ntfy  (must also be True in enabled_alerts)
    "ntfy_events": [
        "Tornado Emergency",
        "Tornado Warning",
        "Tornado Watch",
    ...
    # Tornado emergency config
    "tornado_everyone_count": 10,
    "tornado_everyone_delay": 2,
    ...
    # SMS via Brevo (API preferred, SMTP fallback)
    "sms_enabled":    False,
    "sms_number":     "4027180244",
    "sms_carrier":    "verizon",        # verizon | att | tmobile | sprint | boost | cricket
    "sms_from":       "weather@oofbomb.xyz",
    "sms_from_name":  "weather bot",
    "sms_method":     "api",            # "api" or "smtp"
    "sms_api_key":    "",               # or set via .env BREVO_API_KEY
    "sms_smtp_host":  "smtp-relay.brevo.com",
    "sms_smtp_port":  587,
    "sms_smtp_user":  "a664fc001@smtp-brevo.com",
    "sms_smtp_pass":  "",               # or set via .env BREVO_SMTP_KEY
    "extra_recipients": [],             # extra email/gateway addresses
    ...
    # Seen tracking (internal)
    "_seen_alerts":   [],
    "_seen_products": [],
}

# Inject Brevo keys from environment
_brevo_smtp = os.getenv("BREVO_SMTP_KEY", "")
_brevo_api  = os.getenv("BREVO_API_KEY",  "")
if _brevo_smtp: DEFAULT_CONFIG["sms_smtp_pass"] = _brevo_smtp
if _brevo_api:  DEFAULT_CONFIG["sms_api_key"]   = _brevo_api
...
def load_config() -> dict:
    if CONFIG_FILE.exists():
        try:
            ...
            c.update(saved)
            # Properly deep-merge nested dicts so new keys in DEFAULT don't vanish
            for k in ("enabled_alerts", "iem_products"):
                if k in saved:
                    c[k] = {**DEFAULT_CONFIG[k], **saved[k]}
            return c
...
def save_config(c: dict):
    out = dict(c)
    out["_seen_alerts"]   = c["_seen_alerts"][-500:]
    out["_seen_products"] = c["_seen_products"][-500:]
    CONFIG_FILE.write_text(json.dumps(out, indent=2))

cfg = load_config()

# ── County filter helper ───────────────────────────────────────────────────────
def alert_matches_counties(area_desc: str) -> bool:
    """
    Returns True if the alert affects at least one of the configured counties.
    If no counties are configured, all alerts pass through.
    Matching is case-insensitive substring check against areaDesc.
    """
    counties = cfg.get("counties", [])
    if not counties:
        return True  # no filter configured — allow everything
    area_lower = area_desc.lower()
    return any(c.lower() in area_lower for c in counties)

# ── Notification helpers ───────────────────────────────────────────────────────
async def send_sms_alert(event: str, headline: str, areas: str):
    """
    Send SMS via email-to-carrier gateway.
    Uses Brevo API by default, SMTP as fallback.
    Only fires if the event is BOTH in enabled_alerts AND sms_events.
    """
    if not cfg.get("sms_enabled"):
        return
    # Must be enabled globally
    if not cfg["enabled_alerts"].get(event, False):
        return
    # Must be in the SMS-specific list
    ...
    if method == "api":
        ...
        if not api_key:
            log.warning("SMS: no Brevo API key — set BREVO_API_KEY in .env")
            return
        payload = {
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if r.status in (200, 201):
                        ...
                        log.warning(f"SMS Brevo API error {r.status}: {await r.text()}")
        except Exception as e:
        ...
        if not smtp_pass:
            log.warning("SMS: no SMTP key — set BREVO_SMTP_KEY in .env")
            return
        try:
            import smtplib
...
async def send_ntfy_alert(event: str, headline: str, areas: str):
    """
    Send push notification via ntfy.sh.
    Only fires if the event is BOTH in enabled_alerts AND ntfy_events.
    """
    ...
    icon     = SEVERITY_EMOJI.get(event, "⚠️")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if r.status in (200, 201):
                    log.info(f"ntfy sent: {event} → {server}/{topic}")
                else:
                    log.warning(f"ntfy error {r.status}: {await r.text()}")
    except Exception as e:
...
async def nws_get(session: aiohttp.ClientSession, path: str) -> dict | None:
    ...
    try:
        async with session.get(
            url,
            headers={"User-Agent": "WeatherWatchBot/1.0"},
...
async def fetch_point_forecast(session: aiohttp.ClientSession, lat: float, lon: float) -> str:
    ...
    if not forecast_url:
        return "Forecast unavailable."
    data = await nws_get(session, forecast_url.replace(NWS_BASE, ""))
    if not data:
        return "Forecast unavailable."
    periods = data.get("properties", {}).get("periods", [])[:3]
    return "\n\n".join(f"**{p['name']}**: {p['detailedForecast']}" for p in periods) or "No forecast data."
...
async def check_alerts(session: aiohttp.ClientSession):
    ...
    for alert in alerts:
        ...
        # ── Gate 2: county filter ──────────────────────────────────────────────
        if not alert_matches_counties(areas):
            log.debug(f"Skipped {event} — no matching county in: {areas}")
            continue
        ...
        save_config(cfg)
        log.info(f"New alert: {event} ({aid}) — areas: {areas[:80]}")

        emb = build_alert_embed(alert)

        # ── Ping logic ─────────────────────────────────────────────────────────
        is_everyone = event in cfg.get("everyone_events", [])
        if is_everyone:
            ping_str = "@everyone"
        elif role_id:
            ping_str = f"<@&{role_id}>"
        ...
        try:
            await channel.send(content=ping_str or None, embed=emb)
        except Exception as e:
            log.error(f"Failed to send alert embed: {e}")
            continue

        # ── Notifications (each checks its own event list + enabled_alerts) ───
        asyncio.create_task(send_sms_alert(event, props.get("headline", event), areas))
        asyncio.create_task(send_ntfy_alert(event, props.get("headline", event), areas))

        # ── Tornado emergency repeat pings ─────────────────────────────────────
        if is_everyone and event in ("Tornado Emergency", "Tornado Warning"):
            ...
            for i in range(count - 1):
                ...
                try:
                    await channel.send(
                        f"🚨 **{event.upper()}** 🚨 @everyone — "
                        f"{areas[:100] or 'see above'} ({i+2}/{count})"
                    )
                except Exception:
                    pass
...
class SettingsView(discord.ui.View):
    ...
    def settings_embed(self) -> discord.Embed:
        emb = discord.Embed(
            ...
            description="Configure your weather alert bot. Use the buttons to edit each section.",
            color=0x0099ff,
            timestamp=datetime.now(timezone.utc),
        )
        ...
        counties = cfg.get("counties", [])
        emb.add_field(
            name="🗺️ County Filter",
            value=(
                f"{', '.join(f'**{c}**' for c in counties) if counties else '`Disabled — all alerts pass through`'}\n"
                f"*Alerts not mentioning these counties are ignored*"
            ),
            inline=True,
        )
        ...
        emb.add_field(
            name=f"🚨 Alerts ({len(enabled)} enabled, {len(disabled)} disabled)",
            value="\n".join(f"✅ {e}" for e in enabled[:12]) + ("\n…" if len(enabled) > 12 else "") or "None",
            inline=False,
        )
        ...
        emb.add_field(
            ...
            value=(
                ...
                f"**Events:** {len(cfg.get('ntfy_events',[]))} configured"
            ),
            inline=True,
        )
        ...
        emb.add_field(
            ...
            value=(
                ...
                f"**Events:** {len(cfg.get('sms_events',[]))} configured"
            ),
            inline=True,
        )

        emb.set_footer(text="WeatherWatch · changes save immediately")
        return emb
    ...
    @discord.ui.button(label="🗺️ Counties", style=discord.ButtonStyle.secondary, row=0)
    async def btn_counties(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(CountyModal())
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                ...
                description=(
                    ...
                    "Disabled events are ignored **everywhere** — no embeds, no SMS, no ntfy.\n\n"
                    "To control SMS/ntfy separately, use the 📱 SMS or 🔔 ntfy buttons."
                ),
            ),
    ...
    async def btn_reset(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        save_config(cfg)
        await interaction.response.send_message(
            "✅ Seen history cleared — will repost on next poll.", ephemeral=True)

    @discord.ui.button(label="📱 SMS", style=discord.ButtonStyle.secondary, row=2)
    ...
    @discord.ui.button(label="🧪 Test Alert", style=discord.ButtonStyle.secondary, row=2)
    async def btn_test(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "Use `/test` to send a realistic test alert.", ephemeral=True)
...
class StationModal(discord.ui.Modal, title="Station & Location Settings"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        save_config(cfg)
        await interaction.response.send_message(
            f"✅ Station **{cfg['station']}** · Zone **{cfg['zone']}** · State **{cfg['state']}**",
            ephemeral=True)


class CountyModal(discord.ui.Modal, title="County Filter Settings"):
    counties = discord.ui.TextInput(
        label="Counties to monitor (comma-separated)",
        placeholder="Douglas, Sarpy, Washington",
        style=discord.TextStyle.paragraph,
    )
    note = discord.ui.TextInput(
        label="Leave blank to disable county filter",
        placeholder="If blank, ALL alerts for your zone/state come through",
        required=False,
        max_length=1,
    )
    ...
    def __init__(self):
        super().__init__()
        self.counties.default = ", ".join(cfg.get("counties", ["Douglas", "Sarpy"]))

    async def on_submit(self, interaction: discord.Interaction):
        raw = self.counties.value.strip()
        if raw:
            cfg["counties"] = [c.strip() for c in raw.split(",") if c.strip()]
        else:
            cfg["counties"] = []
        save_config(cfg)
        if cfg["counties"]:
            await interaction.response.send_message(
                f"✅ County filter set: **{', '.join(cfg['counties'])}**\n"
                f"Alerts not mentioning these counties will be ignored.",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
                "✅ County filter **disabled** — all zone/state alerts will come through.",
                ephemeral=True,
            )
...
class ChannelsModal(discord.ui.Modal, title="Discord Channel & Role Settings"):
    ...
    role_id    = discord.ui.TextInput(label="Ping Role ID", placeholder="123456789012345678", required=False)
...
class TornadoModal(discord.ui.Modal, title="Tornado Emergency Settings"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        save_config(cfg)
        await interaction.response.send_message(
            f"✅ **{cfg['tornado_everyone_count']}x** @everyone with **{cfg['tornado_everyone_delay']}s** delay.",
            ephemeral=True,
        )
...
class SmsModal(discord.ui.Modal, title="SMS Alert Settings"):
    ...
    method    = discord.ui.TextInput(label="Method: api or smtp", placeholder="api")
    ...
    def __init__(self):
        ...
        self.number.default   = cfg.get("sms_number", "")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            ...
            f"Key: {key_status}\n"
            f"Events: {', '.join(cfg['sms_events'])}",
            ephemeral=True,
        )


class RecipientsModal(discord.ui.Modal, title="Extra SMS/Email Recipients"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        cfg["extra_recipients"] = [l.strip() for l in self.extras.value.splitlines() if l.strip()]
        save_config(cfg)
        carrier = cfg.get("sms_carrier", "verizon")
        gateway = CARRIER_GATEWAYS.get(carrier, "@vtext.com")
        primary = f"{cfg.get('sms_number','')}{gateway}"
        all_addrs = [primary] + cfg["extra_recipients"]
        await interaction.response.send_message(
            f"✅ **{len(all_addrs)}** total recipients:\n" +
...
class NtfyModal(discord.ui.Modal, title="ntfy Push Notifications"):
    ...
    token   = discord.ui.TextInput(label="Access token (blank if public)", required=False)
    events  = discord.ui.TextInput(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"{status}\n→ `{url}`\nEvents: {len(cfg['ntfy_events'])} configured",
            ephemeral=True,
        )
...
class AlertToggleView(discord.ui.View):
    def __init__(self):
        ...
        sel1 = discord.ui.Select(
            placeholder="Toggle alerts (part 1) — selected = ENABLED",
            options=make_opts(all_alerts[:half]),
            min_values=0,
            max_values=half,
        )
        sel2 = discord.ui.Select(
            placeholder="Toggle alerts (part 2) — selected = ENABLED",
            options=make_opts(all_alerts[half:]),
            min_values=0,
            max_values=len(all_alerts) - half,
        )
        sel1.callback = self._make_callback(all_alerts[:half])
        sel2.callback = self._make_callback(all_alerts[half:])
        self.add_item(sel1)
        self.add_item(sel2)

    def _make_callback(self, keys: list):
        async def callback(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(
                f"✅ **{on}** enabled, **{off}** disabled.", ephemeral=True)
        return callback
...
class ProductToggleView(discord.ui.View):
    _LABELS = {
        ...
        "FFW": "Flash Flood Warning",       "FWW": "Red Flag Warning",
        "FFA": "Flash Flood Watch",         "WOU": "Watch Outline Update",
    }
    ...
    def __init__(self):
        ...
        opts = [
            discord.SelectOption(
                label=f"{k} — {self._LABELS.get(k, k)}",
                value=k,
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ Enabled: {', '.join(on) or 'none'}", ephemeral=True)

# ── Slash commands ─────────────────────────────────────────────────────────────
@tree.command(name="settings", description="Configure WeatherWatch")
@app_commands.checks.has_permissions(manage_guild=True)
async def cmd_settings(interaction: discord.Interaction):
    view = SettingsView()
    await interaction.response.send_message(embed=view.settings_embed(), view=view, ephemeral=True)


@tree.command(name="wx", description="Current NWS forecast for your location")
async def cmd_wx(interaction: discord.Interaction):
    await interaction.response.defer()
    async with aiohttp.ClientSession() as session:
        forecast = await fetch_point_forecast(session, cfg.get("lat", 41.26), cfg.get("lon", -96.01))
    emb = discord.Embed(
    ...
    emb.set_footer(text=f"NWS · {cfg.get('lat')},{cfg.get('lon')}")
...
async def cmd_alerts(interaction: discord.Interaction):
    ...
    counties = cfg.get("counties", [])
    if counties:
        alerts = [a for a in alerts if alert_matches_counties(a.get("properties", {}).get("areaDesc", ""))]

    if not alerts:
        county_note = f" in **{', '.join(counties)}** County" if counties else ""
        await interaction.followup.send(
            embed=discord.Embed(
                title="✅ No active alerts",
                color=0x00cc66,
                description=f"No active NWS alerts{county_note} (zone **{cfg.get('zone','NEZ040')}**).",
            )
        )
        return

    for alert in alerts[:5]:
        await interaction.followup.send(embed=build_alert_embed(alert))


@tree.command(name="forecast", description="Latest Area Forecast Discussion from NWS")
async def cmd_forecast(interaction: discord.Interaction):
    await interaction.response.defer()
    async with aiohttp.ClientSession() as session:
        products = await fetch_iem_products(session, "AFD", cfg.get("station", "OAX"))
    if not products:
        await interaction.followup.send("No recent AFD found.")
        return
    await interaction.followup.send(embed=build_product_embed(products[0], "AFD"))


@tree.command(name="hwo", description="Latest Hazardous Weather Outlook")
async def cmd_hwo(interaction: discord.Interaction):
    ...
    if not products:
        ...
        return
    await interaction.followup.send(embed=build_product_embed(products[0], "HWO"))
...
@tree.command(name="wxstatus", description="Show bot status and config summary")
async def cmd_status(interaction: discord.Interaction):
    ach        = cfg.get("alert_channel_id", 0)
    pch        = cfg.get("product_channel_id", 0)
    on_alerts  = sum(1 for v in cfg["enabled_alerts"].values() if v)
    on_prod    = sum(1 for v in cfg["iem_products"].values() if v)
    counties   = cfg.get("counties", [])
    emb = discord.Embed(title="⛈️ WeatherWatch Status", color=0x0099ff, timestamp=datetime.now(timezone.utc))
    ...
    emb.add_field(name="County Filter",    value=", ".join(counties) if counties else "Off", inline=True)
    ...
    emb.add_field(name="Products Enabled", value=str(on_prod),                           inline=True)
    emb.add_field(name="Seen Alerts",      value=str(len(cfg["_seen_alerts"])),           inline=True)
    emb.add_field(name="SMS",              value="✅" if cfg.get("sms_enabled") else "⛔", inline=True)
    emb.add_field(name="ntfy",             value="✅" if cfg.get("ntfy_enabled") else "⛔",inline=True)
    await interaction.response.send_message(embed=emb)


@tree.command(name="testsms", description="[Admin] Test SMS delivery")
@app_commands.checks.has_permissions(manage_guild=True)
async def cmd_testsms(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    number    = cfg.get("sms_number", "").strip()
    carrier   = cfg.get("sms_carrier", "verizon").lower()
    method    = cfg.get("sms_method", "api")
    gateway   = CARRIER_GATEWAYS.get(carrier, "@vtext.com")
    primary   = f"{number}{gateway}"
    ...
    await interaction.followup.send(
        "\n".join([
            f"**Primary:** `{primary}`",
            f"**Extra:** {', '.join(f'`{r}`' for r in extras) or 'none'}",
            f"**Method:** `{method}`",
            f"**API key:** {'✅' if api_key else '❌ missing'}",
            f"**SMTP key:** {'✅' if smtp_pass else '❌ missing'}",
            "",
            "Sending test…",
        ]),
        ephemeral=True,
    ...
    from_addr = cfg.get("sms_from", "weather@oofbomb.xyz")
    from_name = cfg.get("sms_from_name", "weather bot")
    body      = "weather bot test — SMS is working!"
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    txt = await r.text()
                    if r.status in (200, 201):
                        await interaction.followup.send(f"✅ Sent via API to `{', '.join(all_to)}`", ephemeral=True)
                    else:
                        await interaction.followup.send(f"❌ API error `{r.status}`:\n```{txt[:300]}```", ephemeral=True)
        except Exception as e:
...
@tree.command(name="testntfy", description="[Admin] Send a test ntfy push notification")
@app_commands.checks.has_permissions(manage_guild=True)
async def cmd_testntfy(interaction: discord.Interaction):
    ...
    token   = cfg.get("ntfy_token",  "")
    url     = f"{server}/{topic}"
    headers = {"Title": "weather bot test", "Priority": "high", "Tags": "white_check_mark", "Markdown": "yes"}
    if token:
        headers["Authorization"] = f"Bearer {token}"
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url,
                data="⚠️ **weather bot is working**\n\nYou'll receive real alerts here.\n\n— weather bot".encode("utf-8"),
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=8),
            ) as r:
                txt = await r.text()
                if r.status in (200, 201):
                    await interaction.followup.send(f"✅ ntfy test sent to `{url}`", ephemeral=True)
                else:
                    await interaction.followup.send(f"❌ ntfy error `{r.status}`:\n```{txt[:300]}```", ephemeral=True)
    except Exception as e:
        await interaction.followup.send(f"❌ Exception: `{e}`", ephemeral=True)
...
TEST_EVENTS = {
    "tornado_emergency": {
        ...
        "headline": "TORNADO EMERGENCY — Confirmed large and extremely dangerous tornado near Omaha moving NE at 45 mph",
        "description": (
            "AT 3:47 PM CDT, a TORNADO EMERGENCY has been declared for a large, destructive tornado "
            "near Omaha. TAKE COVER IMMEDIATELY in the lowest level of a sturdy building.\n\n"
            "This tornado is producing catastrophic damage. Flying debris will be deadly."
        ),
        "instruction": "TAKE SHELTER IMMEDIATELY. Move to the lowest floor of a sturdy building. Avoid windows.",
    ...
    "tornado_warning": {
        ...
        "instruction": "Take cover now in a substantial shelter.",
        "severity": "Extreme", "urgency": "Immediate",
    ...
    "severe_thunderstorm": {
        ...
        "description": (
            "AT 3:58 PM CDT, a severe thunderstorm was located near Omaha, moving NE at 30 mph.\n\n"
    ...
    "flash_flood": {
        ...
        "description": "Heavy rain producing flash flooding. Life threatening.",
    ...
    "red_flag": {
        ...
        "headline": "Red Flag Warning in effect until 8:00 PM CDT",
        "description": "Critical fire weather conditions. Strong winds, low humidity.",
        "instruction": "Avoid outdoor burning.",
        "severity": "Severe", "urgency": "Expected",
        "areaDesc": "Antelope [NE], Boone [NE], Burt [NE]",
        "senderName": "NWS Omaha/Valley NE",
    },
}

@tree.command(name="test", description="Send a realistic test alert")
@app_commands.describe(
    event="Type of alert to test",
    silent="No [TEST] label — looks real (default: False)",
    ping_override="Override ping: 'everyone', 'role', 'none', or a count like '5'",
    send_sms="Also fire a real SMS (default: False)",
    bypass_county="Ignore county filter for this test (default: True)",
)
...
async def cmd_test(
    ...
    if not ch:
        await interaction.response.send_message("❌ Can't find alert channel.", ephemeral=True)
        return
    ...
    footer = f"{tdata['senderName']} · WeatherWatch"
    if not silent:
        footer = f"⚠️ TEST ALERT · {footer}"
    emb.set_footer(text=footer)

    is_everyone = event_name in cfg.get("everyone_events", [])
    role_id     = cfg.get("ping_role_id", 0)
    po          = ping_override.strip().lower()

    if po == "none":
        ping_str, do_everyone, override_count = "", False, 0
    elif po == "everyone":
        ping_str, do_everyone, override_count = "@everyone", True, cfg.get("tornado_everyone_count", 10)
    elif po == "role":
        ping_str, do_everyone, override_count = f"<@&{role_id}>" if role_id else "@everyone", False, 0
    elif po.isdigit():
        ping_str, do_everyone, override_count = "@everyone", True, max(1, min(20, int(po)))
    else:
        if is_everyone:
            ping_str, do_everyone = "@everyone", True
        elif role_id:
            ping_str, do_everyone = f"<@&{role_id}>", False
        else:
            ping_str, do_everyone = "", False
    ...
    if do_everyone and override_count > 1:
        ...
        for i in range(override_count - 1):
            await asyncio.sleep(delay)
            try:
                await ch.send(f"🚨 **{event_name.upper()}** 🚨 @everyone — {areas[:100]} ({i+2}/{override_count}){label}")
            except Exception:
                pass
    else:
        await interaction.followup.send(
            f"✅ **{event_name}** → <#{ch_id}> · ping: `{ping_str or 'none'}` · silent={silent}",
            ephemeral=True,
        )
